...
export default function Header({ logoutModalHandler, sidebarHandler, isLogin }) {
  ...
  const handleGetUserInfo = (useCallback(async () => {
	...
	useEffect(() => {
		handleGetUserInfo();
	}, [currentLoginStatus.state, handleGetUserInfo]);
      ...
      <div className="header-wrapper backdrop-blur-sm bg-[#efedfe5e]/60 w-full h-full flex items-center xl:px-0 md:px-10 px-5">
        <div className="flex justify-between items-center w-full">
          ...
          <div className="user-info flex items-center justify-end w-full lg:space-x-7 space-x-2 z-10 ">
            ...
            <Account />
            {isLogin ?
              <AvatarDropdown userName={nickName} profilePhoto={profilePhoto} />
              : (
                <Link to="/login">
                <button
                    type="button"
                    className="btn-login  rounded-[20px] mb-2 text-xl text-white font-bold flex justify-center bg-purple items-center"
                  >
                      <span>Sign In</span>
                  </button>
                  </Link>
